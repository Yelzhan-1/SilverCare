...
export const AlertInbox: React.FC<AlertInboxProps> = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const unsub = alertRepository.subscribeToAlerts((row) => {
      ...
      if (ACTIVE.has(row.status) || row.status === 'acknowledged') {
  ...
  return (
    <section className="bg-clay-surface rounded-clay-lg p-4 shadow-clay-raised-sm space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h3 className="text-sm font-black text-clay-ink">Тревоги подопечного</h3>
          <p className="text-xs text-clay-ink-soft font-semibold">{pushStatus}</p>
        </div>
        <Bell className="w-5 h-5 text-clay-primary shrink-0" />
      </div>

      {error && (
        <p role="alert" className="text-xs font-semibold text-clay-danger">
          {error}
        </p>
      )}
      ...
      {active ? (
        <div
          id="guardian-active-alert"
          className="p-3 rounded-clay-md bg-clay-danger/10 space-y-2"
        >
          <div className="flex items-center gap-2 text-clay-danger font-black text-sm">
            <AlertTriangle className="w-4 h-4" />
            <span>Активная тревога · {statusLabel(active.status)}</span>
          </div>
          <p className="text-xs text-clay-ink-soft">
            {new Date(active.triggered_at ?? '').toLocaleString('ru-RU')}
          </p>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => void alertRepository.acknowledgeAlert(active.id).then(refresh)}
              className="clay-tap min-h-11 px-3 bg-clay-primary text-white text-xs font-bold rounded-xl"
            >
            ...
            <a
              ...
              className="min-h-11 px-3 bg-clay-ink text-white text-xs font-bold rounded-xl inline-flex items-center"
            >
      ...
      <ul className="space-y-1.5 max-h-48 overflow-y-auto">
        {rows.map((row) => (
          <li
            key={row.id}
            className="text-xs font-semibold text-clay-ink flex items-center justify-between gap-2 bg-clay-surface-sunken rounded-xl px-3 py-2"
          >
